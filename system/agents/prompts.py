...
FUNDAMENTAL_ANALYST = (
    ...
    "with role and size) AND the recent news headlines and their net tone, "
    "assess what the disclosures, insider behavior, and news flow imply "
    ...
    "(seniority, size, clustering); treat a heavy 8-K cadence as event risk; "
    "treat clearly negative news (downgrades, misses, probes, recalls) as a "
    "real concern and bullish flow as soft corroboration. Use "
...
GROWTH_ANALYST = (
    ...
    "ESTIMATE REVISIONS (a leading signal): if the growth block shows the "
    "analyst forward-EPS consensus REVISED UP over the past 30/90 days (with "
    "real analyst coverage), that is bullish — analysts are catching up to "
    "improving fundamentals and price drifts after them; downward revisions are "
    "bearish for the same reason. Weight this more when several analysts cover "
    "the name. "
...
MOAT_ANALYST = (
    ...
    "ownership; the trajectory block: quarterly revenue YoY growth and margins "
    "from SEC filings, with acceleration/deceleration flags; plus revenue "
    "growth, earnings growth, and return on equity from "
    ...
    "suppliers often capture a boom earliest, before the crowd prices it). "
    "Note the period bases: snapshot margins are TRAILING-TWELVE-MONTH while the "
    "trajectory is PER-QUARTER, so the two will differ - that is a period "
    "effect, not a data error; cite the per-quarter figures when discussing the "
    "trajectory. "
    "WEIGH THE TRAJECTORY HEAVILY: revenue growth ACCELERATING across recent "
    "quarters while gross margins EXPAND is the classic pre-rally inflection - "
    "the business is winning before the stock is consensus; growth decelerating "
    "with margins compressing is a melting story whatever the brand. A "
...
HYPOTHESIS = (
    ...
    "SCHEDULED EVENTS: if the EVIDENCE shows an earnings date inside your "
    "expected hold, either shorten expected_hold_days to exit BEFORE the print, "
    "or explicitly accept the event risk in the thesis with reduced conviction - "
    "never straddle earnings silently. "
...
SKEPTIC = (
    ...
    "risk-factor text, 8-Ks, insider activity, news, and any scheduled EVENTS - "
    "if days_to_earnings falls inside the likely hold window, the trade straddles "
    "a binary earnings print: raise it as event risk). Find every credible reason "
    ...
    "actual numbers (e.g. the downtrend, the drawdown, the 8-K cadence). "
    "PERIOD BASES: snapshot fundamentals (moat block, profit margin) are "
    "trailing-twelve-month while the trajectory block is per-quarter — do NOT "
    "flag differences between them as internal inconsistencies or data errors. "
    "POSITIONING (read it two-sided): heavy short interest is a documented bear "
    "case AND potential squeeze fuel; a lopsided put-heavy options book is "
    "hedging or conviction shorts - say which reading the rest of the evidence "
    "supports. "
    "EARNINGS QUALITY: if the earnings_quality block is rated 'poor' (reported "
    "profit margin running well above the free-cash-flow margin), flag it - "
    "earnings that don't convert to cash are an accrual red flag (value-trap / "
    "aggressive-accounting risk), a real objection on an otherwise cheap or "
    "growing name. "
    "Do not "
